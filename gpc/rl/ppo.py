...
class BraxPPONetworksWrapper:
    ...
    Allows us to more easily save and load networks with non-default
    architectures.
    """
    ...
    def make_ppo_networks(
        ...
        action_size: int,
        preprocess_observations_fn: types.PreprocessObservationFn = types.identity_observation_preprocessor,  # noqa: E501
        check_sizes: bool = True,
    ) -> PPONetworks:
        """Create a PPONetworks object, compatible with brax's ppo.train().
        ...
        Args:
            ...
            check_sizes: Whether to check that the output sizes of the policy
            and value networks match the action and value distributions.
        ...
        dummy_observation = jnp.zeros((1, observation_size[0]))

        if check_sizes:
            rng = jax.random.PRNGKey(0)

            # Check that the output size of the policy network matches the size
            ...
            assert policy_output.shape[-1] == action_dist.param_size, (
                f"policy network output size {policy_output.shape[-1]} does "
                + f"not match action distribution size {action_dist.param_size}"
            )
            ...
            assert value_output.shape[-1] == 1, (
                f"value network output size {value_output.shape} does not "
                + "match expected size 1"
            )

        # Create the policy network, a FeedForwardNetwork that contains an
        # "init" and an "apply" function.
        def policy_init(key):  # noqa
            """Initialize the policy network from a random key."""
            return self.policy_network.init(key, dummy_observation)

        def policy_apply(processor_params, policy_params, obs):  # noqa
            """Apply the policy given the parameters and an observation."""
            obs = preprocess_observations_fn(obs, processor_params)
            return self.policy_network.apply(policy_params, obs)

        # Create the value network. This is just like the policy network, but
        # with a 1D output.
        def value_init(key):  # noqa
            """Initialize the value network from a random key."""
            return self.value_network.init(key, dummy_observation)

        def value_apply(processor_params, value_params, obs):  # noqa
            """Apply the value function given the params and an observation."""
            obs = preprocess_observations_fn(obs, processor_params)
            return jnp.squeeze(
...
def train_ppo(
    ...
    **kwargs,
    ...
    Args:
        ...
        network_wrapper: A BraxPPONetworksWrapper object containing the policy
        and value networks.
    ...
    Returns:
        make_inference_fn: A function that takes parameters and returns a policy
        function.
    ...
    def progress(num_steps, metrics):  # noqa
...
def make_policy_function(
    ...
    Args:
        network_wrapper: A BraxPPONetworksWrapper object containing the policy
        and value network structures.
        params: The trained parameters.
        observation_size: The size of the observation space.
        action_size: The size of the action space.
        normalize_observations: Whether to normalize observations (e.g. policy
        was trained with normalize_observations=True).
