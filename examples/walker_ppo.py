...
def train(path: str) -> None:
    ...
    save_path = path / "walker_policy.pkl"
    log_path = path / time.strftime("%Y%m%d_%H%M%S")

    # Set up the environment
    episode_length = 500
    env = BraxEnv(WalkerEnv(episode_length))

    # Define value and policy networks (match GPC scale for walker)
    ...
    # Training steps to match GPC:
    # 32 (policy + SPC samples) x 128 (envs) x 500 (ep len) x 20 (iters)
    num_timesteps = 32 * 128 * 500 * 20
...
def test(path: str) -> None:
    ...
    save_path = Path(path) / "walker_policy.pkl"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="/home/vkurtz/gpc_policies/rl_baselines/walker_ppo",
    )
