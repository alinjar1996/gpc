...
def train(path: str) -> None:
    ...
    save_path = path / "crane_policy.pkl"
    log_path = path / time.strftime("%Y%m%d_%H%M%S")

    # Set up the environment
    episode_length = 500
    env = BraxEnv(CraneEnv(episode_length))

    # Define value and policy networks (match GPC scale for crane)
    ...
    # Training steps: 6 (samples) x 512 (envs) x 500 (ep len) x 10 (iters)
    num_timesteps = 6 * 512 * 500 * 10  # 5,120,000
...
def test(path: str) -> None:
    ...
    save_path = Path(path) / "crane_policy.pkl"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="/home/vkurtz/gpc_policies/rl_baselines/crane_ppo",
    )
